...
/*
a[i]*a[j]=i+j
i+j must be multiple of a[i]
find a multiple of a[i] just greater than i
i/a[i] +a[i];
and then check at those indexes whether a[j]*a[i]==i+j
*/
int ans=0;
for(int i=1;i<=n;i++){
    ll mul=((i/a[i])+1)*a[i];
    for(ll k=mul;k<=n+i;k+=a[i]){
        ll j=k-i;
        if(j>i &&j<=n &&a[j]*a[i]==k ){ans++;
        vector<ll>v={i,j,mul,k};
        debug(v);}
      //  debug()
    }
